...
namespace EpgApp.apps.Epg.Services
{
    ...
    {
        private readonly string _baseUri = "https://www.hoerzu.de/text/tv-programm/";
        ...
        public HoerzuDataProviderService(ILogger<Epg> logger)
        {
            _logger = logger;

            _httpClient = new HttpClient { BaseAddress = new Uri(_baseUri) };
            _httpClient.DefaultRequestHeaders.UserAgent.TryParseAdd("Mozilla/5.0 (compatible)");
        }
        ...
        public async Task<IEnumerable<Show>> LoadShowsAsync(string station)
        {
            ...
            {
                _logger.LogError($"Could not find {station} for Hörzu data provider.");
                return shows;
            }
        ...
        private async Task<IEnumerable<Show>> LoadShowsForADayAsync(HoerzuStation station, int day)
        {
            ...
            try
            {
                ...
                var response = await _httpClient.PostAsync($"sender.php?newday={day}&tvchannelid={station.Id}&timeday=ganztags", null);
                ...
                _logger.LogInformation($"Loaded {station.Name} programm for {date}");
            ...
            {
                _logger.LogError(ex, $"Error loading {station.Name} TV program for {(day == 0 ? "today" : "tomorrow")} with HörZu data provider.");
                return shows;
            }
        }
        ...
        {
            return $"{_baseUri.TrimEnd('/')}/detail.php?broadcast_id={show.Id}";
        }

        public async Task<string> GetDescriptionAsMarkdown(Show show)
        {
            var response = await _httpClient.GetAsync($"detail.php?broadcast_id={show.Id}");
            var html = await response.Content.ReadAsStringAsync();
